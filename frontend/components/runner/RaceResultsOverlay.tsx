    ...
    >
      ...
      >
        <GlassPanel className="px-6 py-8 text-center shadow-lift sm:px-8 sm:py-10">
          ...
          <h2 className="mt-2 font-orbitron text-3xl font-black uppercase text-cyan-100 sm:text-4xl">Race complete</h2>
          <p className="mt-1 font-orbitron text-2xl font-bold tabular-nums text-fuchsia-200">{formatRaceTimeMs(result.totalTimeMs)}</p>
          ...
          <div className="mt-8 flex flex-col gap-3">
            <button
              type="button"
              onClick={onRestart}
              className="w-full rounded-2xl border border-cyan-400/40 bg-gradient-to-br from-cyan-500/30 to-slate-950 py-3.5 font-orbitron text-sm font-bold uppercase tracking-[0.15em] text-cyan-50 transition hover:scale-[1.01] active:scale-[0.99]"
            >
              Race again
            </button>
            <Link
            ...
            <Link href="/" className="block text-center font-rajdhani text-xs font-semibold uppercase tracking-[0.2em] text-[var(--text-dim)] hover:text-cyan-200">
              Hub
            </Link>
          </div>
        </GlassPanel>
